...
{
    [self setStartPoint:[[points objectAtIndex:pathNum] CGPointValue]];
    [self setEndPoint:[[points objectAtIndex:pathNum + 1] CGPointValue]];
...
// Let's return an array of CGPoints as values
...
- (NSMutableArray *)pathNodes
{
    ...
    for (Vertex *v in cleanArray) {
        [finalPath addObject:[NSValue valueWithCGPoint:[v coord]]];
    }
...
- (float)distanceFromPoint:(CGPoint)pt toVertex:(Vertex *)dv
{
    float x1 = pt.x;
    float y1 = pt.y;
    float x2 = [dv x];
